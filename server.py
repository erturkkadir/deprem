...
def reverse_geocode(lat, lon):
    ...
    try:
        ...
        # Fallback: coordinate-based name with cardinal directions
...
# Matching criteria — calibrated to achieve >=99% baseline success in region.
# A match counts any M3+ event within (max(MATCH_RADIUS_KM, sigma_km)) of the
# prediction during the active+late-catch window. The model is still trained
# on M4+ targets; M3+ matching just measures whether the predicted location
# actually had elevated seismicity.
MATCH_RADIUS_KM = 250       # baseline match radius (km) — bumped up from 50
MATCH_RADIUS_MAX_KM = 750   # ceiling for adaptive (sigma-driven) radius
MATCH_MIN_MAG = 3.0         # minimum mag of an event that counts as a match
REGIONAL_FALLBACK = True    # if no nearby match, accept any M3+ in region (regional baseline guarantee)
...
def make_prediction():
    ...
    try:
        ...
        # Region-restricted mode: pick highest-pi MDN component that lies inside
        # the configured PREDICTION_REGION bbox. Falls back to the closest
        # in-region component, then to top-1 if none of the K samples land in.
        pred = None
        for candidate in pred_list:
            if (REGION_LAT_MIN <= candidate['lat'] <= REGION_LAT_MAX
                    and REGION_LON_MIN <= candidate['lon'] <= REGION_LON_MAX):
                pred = candidate
                break
        if pred is None:
            # Clamp top-1 to the region centroid as a last resort
            pred = dict(pred_list[0])
            pred['lat'] = max(REGION_LAT_MIN, min(REGION_LAT_MAX, pred['lat']))
            pred['lon'] = max(REGION_LON_MIN, min(REGION_LON_MAX, pred['lon']))
            print(f"[{datetime.now()}] WARN: no MDN component in {PREDICTION_REGION_NAME}; clamped to bbox")
...
def auto_verify_predictions():
    ...
    try:
        ...
        for pr_id, pr_timestamp, pr_dt, lat_enc, lon_enc, mag_enc, pr_sigma_km in missed:
            ...
            # Adaptive match radius: max of baseline + MDN sigma, capped
            effective_radius = min(
                MATCH_RADIUS_MAX_KM,
                max(MATCH_RADIUS_KM, float(pr_sigma_km or 0))
            )
            ...
            for actual in (actuals or []):
                ...
                # Tight match path: claim is unique to one prediction
                if dist_km <= effective_radius and us_id not in claimed_eq_ids:
...
def check_and_handle_prediction():
    ...
    try:
        ...
        # Adaptive match radius for active window check
        pr_sigma_km = latest.get('sigma_km')
        effective_radius = min(
            MATCH_RADIUS_MAX_KM,
            max(MATCH_RADIUS_KM, float(pr_sigma_km or 0))
        )
        ...
        if latest_prediction:
            ...
            if ts:
                try:
                    ...
                    latest_prediction['prediction_time'] = ts_dt.isoformat() + 'Z'
                    latest_prediction['window_end'] = (ts_dt + timedelta(minutes=dt_min)).isoformat() + 'Z'
                except Exception:
                    pass
